    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {ROLES.map((role, i) => (
              <motion.tr key={role.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.08 }}
                className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                <td className="px-4 py-3 sticky left-0 bg-card">
                  <div className="flex items-center gap-2">
                    <Shield size={14} className="text-primary" />
                    <span className="text-sm font-medium text-foreground whitespace-nowrap">{role.name}</span>
                  </div>
                </td>
                {MODULES.map(m => (
                  <td key={m} className="px-3 py-3 text-center">
                    <div className="flex justify-center">
                      <PermCell perm={(role.perms as Record<string, string>)[m] || ''} />
                    </div>
                  </td>
                ))}
              </motion.tr>
